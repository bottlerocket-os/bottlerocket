...
fn get_target_version(config: &Config, force_version: &Option<Version>) -> Result<Option<Version>> {
    match force_version {
        Some(version) => Ok(Some(version.to_owned())),
        // If no version is specified on the commandline, get the version from the config
        None => {
            if config.version_lock != "latest" {
                match FriendlyVersion::try_from(config.version_lock.as_str()) {
                    Ok(version) => Ok(Some(version.try_into().context(error::BadVersion)?)),
                    Err(_) => Ok(None),
                }
            } else {
                Ok(None)
    ...
    match command {
        Command::CheckUpdate | Command::Whats => {
            ...
            let update = update_required(
                ...
                &variant,
                get_target_version(&config, &arguments.force_version)?,
            )
            .context(error::UpdateNotAvailable)?;

            if !(arguments.ignore_waves || config.ignore_waves) {
                ensure!(
        ...
        Command::Update | Command::UpdateImage => {
            if let Some(u) = update_required(
                ...
                if u.update_ready(config.seed) || (arguments.ignore_waves || config.ignore_waves) {
