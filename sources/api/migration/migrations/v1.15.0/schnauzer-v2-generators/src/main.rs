    ...
    #[cfg(variant_platform = "aws")]
    {
        migrations.append(&mut vec![
            MetadataReplacement {
                ...
                new_val: "schnauzer-v2 render --requires 'aws@v1(helpers[ecr-prefix])' --template '{{ ecr-prefix settings.aws.region }}/bottlerocket-admin:v0.10.1'",
            },
            MetadataReplacement {
                setting: "settings.host-containers.control.source",
                metadata: "setting-generator",
                old_val: "schnauzer settings.host-containers.control.source",
                new_val: "schnauzer-v2 render --requires 'aws@v1(helpers=[ecr-prefix])' --template '{{ ecr-prefix settings.aws.region }}/bottlerocket-control:v0.7.2'",
            },
            MetadataReplacement {
