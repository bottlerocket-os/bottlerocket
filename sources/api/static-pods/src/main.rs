...
struct StaticPod {
    ...
    enabled: Option<bool>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    manifest: Option<ValidBase64>,
}

type Result<T> = std::result::Result<T, error::Error>;

/// Query the API for the currently defined static pods
fn get_static_pods<P>(config_path: P) -> Result<Option<HashMap<Identifier, StaticPod>>>
where
    P: AsRef<Path>,
{
    debug!("Loading static pod information from configuration file");
...
fn handle_static_pod<S>(name: S, pod_info: &StaticPod) -> Result<()>
where
    S: AsRef<str>,
{
...
fn run() -> Result<()> {
    ...
    let mut failed = 0u32;
    if let Some(static_pods) = get_static_pods(args.config_path)? {
...
fn parse_args(args: env::Args) -> Result<Args> {
                ...
                config_path = Some(
                    iter.next()
                        .ok_or_else(|| error::Error::Usage {
                            message: "Did not give argument to --config-path".into(),
                        })?
                        .into(),
                )
            }
    ...
    Ok(Args {
        ...
        config_path: config_path.unwrap_or_else(|| DEFAULT_CONFIG_PATH.into()),
    })
}
...
mod error {
    ...
    pub(super) enum Error {
        ...
        #[snafu(display("Failed to read static-pods configuration file at '{}': {}", path.display(), source))]
        ReadConfig {
            path: PathBuf,
            source: std::io::Error,
        },

        #[snafu(display("Failed to deserialize static pod configuration at '{}': {}", path.display(), source))]
        Deserialization {
            path: PathBuf,
