...
func (*binExecute) execute(ctx context.Context, args []string, data interface{}) (interface{}, error) {
	...
	if err := cmd.Start(); err != nil {
		return nil, errors.WithMessagef(err, "%q failed to execute", updogBin)
	}
	if err := json.NewDecoder(stdout).Decode(data); err != nil {
		return nil, errors.WithMessagef(err, "could not parse %q stdout", updogBin)
	}
	err = cmd.Wait()
	return data, errors.WithMessagef(err, "%q errored", updogBin)
}

func (u *updog) Status() (*statusResponse, error) {
	action := CommandStatusQuery
	shape := &statusResponse{Action: action}
	response, err := u.cli.execute(context.TODO(), []string{action.String()}, shape)
	if err != nil {
