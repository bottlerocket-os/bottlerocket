...
%global user_data_provider local-overrides
...
%package -n %{_cross_os}local-overrides-user-data-client
Summary: Local overrides user data client
%description -n %{_cross_os}local-overrides-user-data-client
%{summary}.

# Symlinks the binary to the appropriate .d directory as 50-
%package -n %{_cross_os}local-overrides-user-data-provider
Summary: Local overrides user data provider config
%description -n %{_cross_os}local-overrides-user-data-provider
%{summary}.
...
ln -sf \
    ...
    %{buildroot}%{_cross_datadir}/early-boot-config/data-providers.d/50-%{user_data_provider}


%files -n %{_cross_os}local-overrides-user-data-client
%{_cross_libexecdir}/early-boot-config/bin/%{user_data_provider}-provider

%files -n %{_cross_os}local-overrides-user-data-provider
